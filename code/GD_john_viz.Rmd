...
## Unemployment
see ses_ancestry_pums.Rmd
```{r}
ddi <- read_ipums_ddi("../data/raw/pums/usa_00001.xml")
pums_data <- read_ipums_micro(ddi, verbose = FALSE)
```
```{r}
pums <- pums_data %>%
	mutate_at(vars(OWNERSHP, RACE, HISPAN, BPL, ANCESTR1, ANCESTR1D, EDUC, EDUCD,
								 EMPSTAT, EMPSTATD, LABFORCE, VEHICLES, AGE), as_factor) %>%
	...
	mutate(VEHICLES = as.character(VEHICLES)) %>%
	mutate(VEHICLES = if_else(VEHICLES == "6 (6+, 2000, ACS and PRCS)", "6", VEHICLES)) %>%
	mutate(VEHICLES = if_else(VEHICLES == "No vehicles available", "0", VEHICLES)) %>%
	mutate(VEHICLES = if_else(VEHICLES == "1 available", "1", VEHICLES)) %>%
	mutate(VEHICLES = as.integer(VEHICLES)) %>%
	left_join(pob, by = c("BPL" = "state_name")) %>% 
	mutate(birthplace = replace_na(birthplace, "Foreign born")) %>% 
	mutate(birthplace = as.factor(birthplace) %>% 
				 	fct_relevel("Born in US", "Born in US Territory", "Foreign born")) %>% 
	mutate(AGE = as.character(AGE)) %>%
	mutate(AGE = if_else(AGE == "Less than 1 year old", "0.5", AGE)) %>%
	mutate(AGE = if_else(AGE == "90 (90+ in 1980 and 1990)", "90", AGE)) %>%
	mutate(AGE = as.numeric(AGE)) %>%
	left_join(ancestry_grps, by = c("ANCESTR1", "ANCESTR1D")) %>% 
	left_join(ed_levels, by = c("EDUC", "EDUCD")) %>% 
	mutate(EDU2 = as.factor(EDU2) %>% 
				 	fct_relevel("Less than HS", "HS or GED" , "Some college / AA",
...
adds <- read_csv("../data/clean/nhv_addresses.csv") %>% 
  dplyr::select(-`Unique ID`, -City, -State, -ZIP)
properties <- read_csv("../data/clean/property_info.csv")
rawproperties <- read_sf(dsn="../data/clean/arcgis_output/nhv_addresses_GeocodeAddress.shp") %>% 
  bind_cols(adds) %>% 
  bind_cols(select(full_df, -address, -owner))

saveRDS(rawproperties, "../data/clean/use_this.RDS")
