...
* 1933: in past 2 years, university has increased tax-exempt land by ~20 percent
...
yale_cum <- yale_annual %>% 
  filter(year_built >= 1929 & year_built <= 1935) %>% 
  mutate(ass_cum = cumsum(assessment)) %>% 
  select(year_built, ass_cum) %>% 
  mutate(pct_growth = (ass_cum - 32.69777)/32.69777)
```
```{r}
...
plot1 <- yale_annual %>% ggplot(aes(x=year_built, y=assessment)) +
  geom_rect(aes(xmin = 1928.75, xmax = 1935.25, ymin = 0, ymax = Inf, fill = "GREEN", color="GREEN"), alpha = 0.03) +
...
```{r}
plot1 + 
  annotate(geom = "segment", x = 1931.1, y = 50, xend = 1931.03, yend = 61) +
  annotate(geom = "text", x = 1929.5, y = 38, 
           label = "About 10,000 New Haven\nresidents (6.2%) are unemployed", hjust = "left", size=3)  + 
  annotate(geom = "segment", x = 1933, y = 194, xend = 1932.2, yend = 194) +
  annotate(geom = "text", x = 1933.1, y = 194, 
           label = "Yale builds HGS, PWG, Grace Hopper", hjust = "left", size=3) +
 annotate(geom = "segment", x = 1934, y = 101.80583, xend = 1933.2, yend = 101.80583) +
  annotate(geom = "text", x = 1934.1, y = 101.80583, 
           label = "In the past 2 years, Yale has increased\ntax-exempt land by ~20%", 
           hjust = "left", size=3)
