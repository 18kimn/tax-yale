...
```{r, eval=FALSE}
emp_ddi <- read_ipums_ddi("../data/raw/pums/usa_00001.xml")
emp_pums_data <- read_ipums_micro(ddi, verbose = FALSE)
emp_pums <- pums_data %>%
	mutate_at(vars(EMPSTAT, EMPSTATD), as_factor) %>%
	mutate_at(vars(PERWT, HHWT), as.numeric) %>%
	janitor::clean_names()
emp_des <- pums %>% 
	filter(empstat != "N/A") %>% 
	as_survey_design(., ids = 1, wt = perwt) %>%
	filter(!grepl("Armed", empstatd),
				 empstat != "Not in labor force")

save(emp_des, file="../data/raw/pums/employment_des.RData")
```
```{r}
load("../data/raw/pums/employment_des.RData")
emp_counts <- emp_des  %>% 
	select(year, perwt, empstat) %>% 
	group_by(year, empstat) %>% 
	summarise(count = survey_total(perwt)) %>% 
  ungroup() %>% 
